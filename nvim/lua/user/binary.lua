...
vim.api.nvim_create_user_command("BinaryView", function(opts)
    local buf = vim.api.nvim_get_current_buf()
    local path = vim.b[buf].binary_preview_source or vim.api.nvim_buf_get_name(buf)

    if path == "" or vim.fn.isdirectory(path) == 1 or not is_binary(path) then
        vim.notify("Current buffer is not a binary file", vim.log.levels.WARN)
        return
    end
    ...
    if mode == "auto" then
        cmd, ft = preview_command(path)
    elseif mode == "disasm" then
        cmd, ft = disasm_command(path)
    elseif mode == "strings" then
        cmd, ft = "strings -a " .. shellescape(path) .. " 2>/dev/null", "text"
    elseif mode == "hex" then
        cmd, ft = "xxd " .. shellescape(path) .. " 2>/dev/null", "xxd"
    elseif mode == "headers" then
        cmd, ft = headers_command(path)
    else
        vim.notify("BinaryView modes: auto, disasm, strings, hex, headers", vim.log.levels.WARN)
        return
    end

    if not cmd then
        vim.notify("No compatible preview command found for this binary", vim.log.levels.WARN)
        return
    end

    local lines = systemlist(cmd)
    if not lines then
        vim.notify("Failed to run preview command: " .. cmd, vim.log.levels.ERROR)
        return
    end
    ...
    nargs = "?",
    complete = function()
        return { "auto", "disasm", "strings", "hex", "headers" }
    end,
})
